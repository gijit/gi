...
func (p *parser) init(base *PosBase, r io.Reader, errh ErrorHandler, pragh PragmaHandler, fileh FilenameHandler) {
	p.base = base
	p.errh = errh
	p.fileh = fileh
	p.scanner.init(
...
const lineMax = 1<<24 - 1 // TODO(gri) this limit is defined for Pos - fix
...
func (p *parser) updateBase(line, col uint, text string) {
	...
	filename := text[:i]
	p.base = NewLinePragmaBase(MakePos(p.base.Pos().Base(), line, col), filename, uint(n))
}
...
func (p *parser) fileOrNil() *File {
	...
	*/
	...
	if p.first != nil {
		pp("p.first != nil, is '%#v', returning early. 'don't bother continuing if package clause has errors' <- but may not have to do with package, now at the repl...", p.first)
		return nil
	}

	// { ImportDecl ";" }
	for p.got(_Import) {
		f.NodeList = p.appendGroupNode(f.NodeList, p.importDecl)
		p.want(_Semi)
	}
	...
	for p.tok != _EOF {
		...
		case _Const:
			p.next()
			f.NodeList = p.appendGroupNode(f.NodeList, p.constDecl)

		case _Type:
			pp("jea debug, top level type")
			p.next()
			f.NodeList = p.appendGroupNode(f.NodeList, p.typeDecl)

		case _Var:
			p.next()
			f.NodeList = p.appendGroupNode(f.NodeList, p.varDecl)
		...
		case _Func:
			...
			/*			// jea add
						case _Literal:
							pp("jea _Literal: p.tok = '%#v'", p.tok) // 0x3 is _Literal
							p.next()
							d := p.expr()
							pp("jea: _literal: d = '%#v'", d)
							if d != nil {
								f.NodeList = append(f.NodeList, d)
							}
			*/
			// jea add
		case _Name, _Literal, _Switch, _If, _Select, _For:
			d := p.stmtOrNil()
			if d != nil {
				f.NodeList = append(f.NodeList, d)
			}
		...
		default:
			pp("jea default p.tok = '%#v'/'%v'", p.tok, tokstring(p.tok)) // 0x3
			if p.tok == _Lbrace && len(f.NodeList) > 0 && isEmptyFuncDecl(f.NodeList[len(f.NodeList)-1]) {
		...
		pp("fileOrNil past the switch on p.tok")
		...
		if p.tok != _EOF && !p.got(_Semi) {
			// top level expression of "3" is throwing here. Also "3+4" is throwing here.
	...
	pp("jea debug: %v is p.tok =?= _EOF: %v.  p.pos()='%v', line=%v, col=%v, pos.base=%v", p.tok, p.tok == _EOF, pos, pos.Line(), pos.Col(), pos.base)

	if base == nil && line == 1 && col == 1 {
		panic(EmptyInput)
	}
...
func isEmptyFuncDecl(dcl Node) bool {
	f, ok := dcl.(*FuncDecl)
...
func (p *parser) appendGroupNode(list []Node, f func(*Group) Node) []Node {
	if p.tok == _Lparen {
		g := new(Group)
...
func (p *parser) importDecl(group *Group) Node {
	if trace {
...
func (p *parser) constDecl(group *Group) Node {
	if trace {
...
func (p *parser) typeDecl(group *Group) Node {
	if trace {
...
func (p *parser) varDecl(group *Group) Node {
	if trace {
...
// jea add
func (p *parser) basicLit(group *Group) Node {
	if trace {
		defer p.trace("basicLit")()
	}

	d := new(BasicLit)
	d.pos = p.pos()

	/* jea TODO what goees here? just copied from varDecl above.
	d.NameList = p.nameList(p.name())
	if p.got(_Assign) {
		d.Values = p.exprList()
	} else {
...
func (p *parser) funcBody() *BlockStmt {
	...
	/* jea in slimming dependencies, we didn't include the
		         branches.go branch/goto labels checking file.
	           Maybe restore it later, but fine without it for now.
...
func (p *parser) unaryExpr() Expr {
	...
	pp("jea debug: about to call p.pexpr() at the end of unaryExpr")
...
func (p *parser) operand(keep_parens bool) Expr {
	if trace {
		...
		//panic(fmt.Sprintf("jea debug: where is this operand "+
		//	"called from? p.tok=%v/%v", p.tok, tokstring(p.tok)))
	}
	...
	case _Literal:
		res := p.oliteral()
		pp("jea debug: p.tok _Literal, in operand(), p.oliteral() gave '%#v'", res)
		return res
	...
	default:
		if p.tok == _EOF {
			pp("jea: in operand: expected expression, got EOF!! p.tok= '%#v' at pos %v", p.tok, p.pos())
			panic(ErrMoreInput)
		} else {
			pp("jea: in operand, NOT EOF.: expected expression, might/might not have an expression. tok= '%s'/%#v at pos %v", p.tok, p.tok, p.pos())
			pp("jea: p.tok is decimal %d/string: '%s'. with value='%v'", p.tok, tokstring(p.tok), p.tok.String())
		}
		switch p.tok {
		case _Semi:
			pp("jea: _Semi: complete no error.")
			panic(CompleteNoError)
		case _Operator:
			pp("jea: operator... more input??")
			panic(ErrMoreInput)
		}
		x := p.bad()
...
func (p *parser) declStmt(f func(*Group) Node) *NodeStmt {
	if trace {
	...
	s.NodeList = p.appendGroupNode(nil, f)

	return s
}

func (p *parser) nodeStmt(f func(*Group) Node) *NodeStmt {
	if trace {
		defer p.trace("nodeStmt")()
	}
...
func (p *parser) stmtOrNil() Stmt {
	...
	case _Var:
		return p.nodeStmt(p.varDecl)

	case _Const:
		return p.nodeStmt(p.constDecl)

	case _Type:
		return p.nodeStmt(p.typeDecl)
