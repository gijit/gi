...
//
func Parse(base *PosBase, src io.Reader, errh ErrorHandler, pragh PragmaHandler, fileh FilenameHandler) (_ *File, first error) {
	defer func() {
		// jea debug, so we see where the error was...
		return
		if p := recover(); p != nil {
	...
	var p parser
	p.init(base, src, errh, pragh, fileh)
	p.next()
	return p.fileOrNil(), p.first
}

// ParseBytes behaves like Parse but it reads the source from the []byte slice provided.
func ParseBytes(base *PosBase, src []byte, errh ErrorHandler, pragh PragmaHandler, fileh FilenameHandler, mode Mode) (*File, error) {
	return Parse(base, &bytesReader{src}, errh, pragh, fileh)
}
...
func ParseFile(filename string, errh ErrorHandler, pragh PragmaHandler) (*File, error) {
	...
	defer f.Close()
	return Parse(NewFileBase(filename, filename), f, errh, pragh, nil)
}
