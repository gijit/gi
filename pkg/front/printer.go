...
func groupFor(d Node) (token, *Group) {
	switch d := d.(interface{}).(type) {
	case *ImportDecl:
	...
	// jea add
	case *Expr:
		panic("jea TODO: Expr groupFor")
	case *BasicLit:
		panic("jea TODO: BasicLit groupFor")
	case *CompositeLit:
		panic("jea TODO: CompositeLit groupFor")
	case *FuncLit:
		panic("jea TODO: FuncLit groupFor")

	default:
		panic(fmt.Sprintf("jea TODO: what type?: d=%T/val='%#v'", d, d))
		panic("unreachable")
	}
}
...
func (p *printer) printNodes(list []Node) {
...
func (p *printer) printNodeList(list []Node) {
	i0 := 0
	var tok token
	var group *Group
	for i, x := range list {
