...
func TestSourcePos(t *testing.T) {
	const src = `
package p
import ( "github.com/go-interpreter/gi/pkg/more/printer"; "math" )
...
func TestDeclLists(t *testing.T) {
	for _, src := range decls {
		...
		var buf bytes.Buffer
		err = Fprint(&buf, fset, file.Nodes) // only print declarations
		if err != nil {
			panic(err) // error in test
		}
...
func TestStmtLists(t *testing.T) {
	for _, src := range stmts {
		...
		var buf bytes.Buffer
		err = Fprint(&buf, fset, file.Nodes[0].(*ast.FuncDecl).Body.List) // only print statements
		if err != nil {
			panic(err) // error in test
		}
...
`
	...
	err = Fprint(&buf, fset, &CommentedNode{Node: f.Nodes[0], Comments: f.Comments})
	if err != nil {
		t.Fatal(err)
	}
	...
	err = Fprint(&buf, fset, &CommentedNode{Node: f.Nodes[1], Comments: f.Comments})
	if err != nil {
		t.Fatal(err)
	}
