...
import (
	"bytes"
	"fmt"
	"github.com/go-interpreter/gi/pkg/more/priv/testenv"
	"github.com/go-interpreter/gi/pkg/more/ast"
	"github.com/go-interpreter/gi/pkg/more/importer"
...
func pkgFor(path, source string, info *Info) (*Package, *Checker, error) {
	...
	return conf.Check(nil, nil, f.Name.Name, fset, []*ast.File{f}, info)
}

func mustTypecheck(t *testing.T, path, source string, info *Info) string {
	pkg, _, err := pkgFor(path, source, info)
	if err != nil {
		name := path
		if pkg != nil {
...
func TestMultiFileInitOrder(t *testing.T) {
	...
	for _, test := range []struct {
		...
		var info Info
		if _, _, err := new(Config).Check(nil, nil, "main", fset, test.files, &info); err != nil {
			t.Fatal(err)
		}
...
func TestSelection(t *testing.T) {
	...
	makePkg := func(path, src string) {
		...
		pkg, _, err := conf.Check(nil, nil, path, fset, []*ast.File{f}, &Info{Selections: selections})
		if err != nil {
			t.Fatal(err)
		}
		imports[path] = pkg
...
func TestIssue8518(t *testing.T) {
	...
	makePkg := func(path, src string) {
		...
		pkg, _, _ := conf.Check(nil, nil, path, fset, []*ast.File{f}, nil) // errors logged via conf.Error
		imports[path] = pkg
	}
...
func TestLookupFieldOrMethod(t *testing.T) {
	...
	for _, test := range tests {
		pkg, _, err := pkgFor("test", "package p;"+test.src, nil)
		if err != nil {
...
func TestScopeLookupParent(t *testing.T) {
	...
	makePkg := func(path string, files ...*ast.File) {
		var err error
		imports[path], _, err = conf.Check(nil, nil, path, fset, files, &info)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestIssue15305(t *testing.T) {
	...
	conf.Check(nil, nil, "p", fset, []*ast.File{f}, info) // ignore result
	for e, tv := range info.Types {
...
func TestCompositeLitTypes(t *testing.T) {
	for _, test := range []struct {
		...
		if _, _, err = new(Config).Check(nil, nil, "p", fset, []*ast.File{f}, info); err != nil {
			t.Fatalf("%s: %v", test.lit, err)
		}
		...
		rhs := f.Nodes[0].(*ast.GenDecl).Specs[0].(*ast.ValueSpec).Values[0]
...
import foo "github.com/go-interpreter/gi/pkg/more/types/thisdirectorymustnotexistotherwisethistestmayfail/foo" // should only see an error here
...
`
	...
	for _, compiler := range []string{"gc", "gccgo", "source"} {
		...
		pkg, _, _ := conf.Check(nil, nil, "p", fset, files, info)
		if pkg == nil {
