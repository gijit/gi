...
func TestColonEqualsScope(t *testing.T) {
	...
	as := f.Nodes[0].(*ast.FuncDecl).Body.List[0].(*ast.AssignStmt)
...
func TestVarScope(t *testing.T) {
	...
	// RHS refers to undefined globals; LHS does not.
	as := f.Nodes[0].(*ast.FuncDecl).Body.List[0].(*ast.DeclStmt).Decl.(*ast.GenDecl).Specs[0].(*ast.ValueSpec)
...
`
	...
	comment := f.Nodes[0].(*ast.GenDecl).Specs[0].(*ast.TypeSpec).Comment.List[0].Text
	if comment != "// comment" {
