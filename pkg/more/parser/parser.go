...
type File struct {
	...
	// At the repl, the order and interleaving
	// of declarations and assignments is significant.
	// Hence we change to a sequence of Nodes,
	// rather than just of declarations.

	Nodes []ast.Node // top-level statements, expressions, decls; or nil
...
func (p *parser) next() {
	...
	prev := p.pos
	p.next0()
	//fmt.Printf("jea debug: p.next0() produced token: p.tok= '%v'/ p.lit='%s' at p.pos=%v\n", p.tok, p.lit, p.pos)
...
func (p *parser) error(pos token.Pos, msg string) {
	...
	fmt.Printf("jea debug, epos = '%#v' in error()\n")
	panic(ErrSyntax)
...
func (p *parser) errorExpected(pos token.Pos, msg string) {
	...
	// jea: can't always return ErrMoreInput on p.tok == EOF, hmmm...
	if p.tok == token.EOF {
		fmt.Printf("\n last token was EOF\n")
		fmt.Printf("\n scanner.IsEOF says %v \n", p.scanner.IsEOF())
		//		panic(ErrMoreInput)
	}
	panic(ErrSyntax) // jea
...
func assert(cond bool, msg string) {
	if !cond {
		panic("github.com/go-interpreter/gi/pkg/more/parser internal error: " + msg)
	}
}
...
func (p *parser) parseOperand(lhs bool) ast.Expr {
	...
	// if actual tok is EOF versus the 2nd '*' in a row, that's an important distinction.
	fmt.Printf("debug, at parser.go:1204, p.tok = '%#v'. is eof? %v\n", p.tok, p.tok == token.EOF)
	pos := p.pos
...
func (p *parser) parseFile() *ast.File {
	...
	if p.mode&PackageClauseOnly == 0 {
		...
		for p.tok == token.IMPORT {
			nodes = append(nodes, p.parseGenDecl(token.IMPORT, p.parseImportSpec))
		}
