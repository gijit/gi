...
func FilterDecl(decl Decl, f Filter) bool {
	return filterNode(decl, f, false)
}

func filterNode(node Node, f Filter, export bool) bool {
	switch d := node.(type) {
	case *GenDecl:
...
func filterFile(src *File, f Filter, export bool) bool {
	...
	for _, d := range src.Nodes {
		if filterNode(d, f, export) {
			src.Nodes[j] = d
			j++
		}
	}
	src.Nodes = src.Nodes[0:j]
	return j > 0
}
