...
func Test015ArrayCreation(t *testing.T) {
	...
	cv.Convey("creating arrays via x := [3]int{1,2,3} where `x` is a slice should compile", t, func() {
		...
		cv.So(string(inc.Tr([]byte(code))), cv.ShouldMatchModuloWhiteSpace, `x=_gi_NewArray({[0]=1LL,2LL,3LL}, "int", 3, 0LL); bb = 3LL;`)

		// and empty array with size 3

		code = `var x [3]int`
		cv.So(string(inc.Tr([]byte(code))), cv.ShouldMatchModuloWhiteSpace, `x=_gi_NewArray({}, "int", 3, 0LL);`)
