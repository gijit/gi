...
func Test041RangeOverUtf8BytesInString(t *testing.T) {
	...
	cv.Convey(`From the https://blog.golang.org/strings blog example of a for-range loop over utf8 in strings: Given the string nihongo := "日本語" containing three multibyte utf8 characters and, the loop: for i, runeValue := range s { println(r); } should loop over whole utf8 runes and not individual bytes. So runeValue should take on each of the three full runes in turn.`, t, func() {

		// Resources:
		//
		// Rob Pike's blog: https://github.com/go-interpreter/gi/blob/master/pkg/utf8/utf8.go
		//
		// see https://github.com/go-interpreter/gi/issues/13 for discussion, and to note
		//  progress and completion.
		//
		// See the lua code in https://github.com/go-interpreter/gi/blob/master/pkg/utf8/utf8.lua
		// See the Go code in https://github.com/go-interpreter/gi/blob/master/pkg/utf8/utf8.go
