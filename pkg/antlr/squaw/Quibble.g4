...
// javascript style, does not suffice for Go.
eos
    : ';'?
    | EOF
...
/// boundary, below = hand written for Quibble, above is from Gofront.

// using our assignment and expression, the eof vs syntax error works:
...
constSpec
    : identifierList ( ltype? '=' expressionList )?
    ;
...
varSpec
    : identifierList ( ltype ( '=' expressionList )? | '=' expressionList )
    ;
...
fieldDecl
    : (identifierList ltype | anonymousField) STRING_LIT?
    ;
...
arguments
    : '(' ( ( expressionList | ltype ( ',' expressionList )? ) '...'? ','? )? ')'
    ;
...
expression
    : unaryExpr
    // jea: things break with BINARY_OP that don't break when we manually inline it.
    //  calc_test.go line 24, the "3 * 4" test, for example.
