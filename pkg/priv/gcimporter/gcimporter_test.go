...
func TestImportTestdata(t *testing.T) {
	...
	if pkg := testPath(t, "./testdata/exports", "."); pkg != nil {
		...
		for _, want := range []string{"github.com/glycerine/gi/pkg/ast", "github.com/glycerine/gi/pkg/token"} {
...
func TestIssue13898(t *testing.T) {
	...
	imports := make(map[string]*types.Package)
	_, err := Import(imports, "github.com/glycerine/gi/pkg/internal/gcimporter", ".", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	for path, pkg := range imports {
		if path == "github.com/glycerine/gi/pkg/types" {
			goTypesPkg = pkg
			break
		}
	}
	if goTypesPkg == nil {
		t.Fatal("github.com/glycerine/gi/pkg/types not found")
	}
	...
	if !ok {
		t.Fatalf("github.com/glycerine/gi/pkg/types.Object type is %v; wanted named type", typ)
	}

	// lookup go/types.Object.Pkg method
	m, index, indirect := types.LookupFieldOrMethod(typ, false, nil, "Pkg")
	if m == nil {
		t.Fatalf("github.com/glycerine/gi/pkg/types.Object.Pkg not found (index = %v, indirect = %v)", index, indirect)
	}

	// the method must belong to go/types
	if m.Pkg().Path() != "github.com/glycerine/gi/pkg/types" {
